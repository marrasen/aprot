...
// caller's context; read the caller's identity from
// [aprot.PrincipalFrom], which is populated the same way on every
// transport. (aprot.Connection(ctx) is non-nil only on socket transports —

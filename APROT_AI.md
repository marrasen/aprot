...
- Works with both `Register` and `RegisterREST` groups. `RegisterREST` + `EnableMCP` = tools over MCP (and REST) with no WebSocket dispatch. Caveat: the TS generator emits client functions for every registered group (no REST/MCP filter), so REST-only groups still appear in the generated client even though their socket calls cannot succeed.

...
// Uses raw string constants matching the template output to avoid import cycles.
func testTasksHook(results map[string]string, mode OutputMode, hasMeta bool) {
	_, hasHandlerFile := results["task-cancel-handler.ts"]

	const cancelFn = `export function cancelSharedTask(client: ApiClient, taskId: string): Promise<void> {
...
`
	...
	code.WriteString(cancelFn)
	code.WriteString("\n")

	if mode == OutputReact {
		code.WriteString(reactHooks)
	}

	if hasMeta {
		code.WriteString(metaSuffix)
	}

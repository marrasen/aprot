...
// taskCancelHandler mirrors tasks.taskCancelHandler for generator tests.
// Defined here to avoid a circular import (aprot → tasks → aprot).
...
// enableTestTasks sets up the task system the same way tasks.Enable() does.
func enableTestTasks(r *Registry) {
	...
	r.OnGenerate(func(results map[string]string, mode OutputMode) {
		testTasksHook(results, mode, false)
	})
}

// enableTestTasksWithMeta sets up the task system the same way tasks.EnableWithMeta() does.
func enableTestTasksWithMeta[M any](r *Registry) {
	r.SetTaskMetaType(reflect.TypeFor[M]())
	r.SetTasksEnabled(true)
	r.RegisterEnum(TaskNodeStatusValues())
	handler := &taskCancelHandler{}
	r.Register(handler)
	...
	r.RegisterPushEventFor(handler, TaskUpdateEvent{})
	r.OnGenerate(func(results map[string]string, mode OutputMode) {
		testTasksHook(results, mode, true)
	})
}

// testTasksHook mirrors the convenience code generated by tasks.appendTaskConvenienceCode.
func testTasksHook(results map[string]string, mode OutputMode, hasMeta bool) {
	_, hasHandlerFile := results["task-cancel-handler.ts"]

	var code strings.Builder
	if hasHandlerFile {
		code.WriteString("// Code generated by aprot. DO NOT EDIT.\n\n")
		if mode == OutputReact {
			code.WriteString("import { useState, useEffect, useCallback } from 'react';\n")
			code.WriteString("import type { ApiClient } from './client';\n")
			code.WriteString("import { useApiClient } from './client';\n")
			code.WriteString("import type { SharedTaskState, TaskStateEvent, TaskUpdateEvent } from './task-cancel-handler';\n\n")
		} else {
			code.WriteString("import type { ApiClient } from './client';\n")
			code.WriteString("import type { SharedTaskState, TaskStateEvent, TaskUpdateEvent } from './task-cancel-handler';\n\n")
		}
	...
	code.WriteString("export function cancelSharedTask(client: ApiClient, taskId: string): Promise<void> {\n")
	code.WriteString("    return client.request<void>('taskCancelHandler.CancelTask', [{ taskId }]);\n")
	code.WriteString("}\n")

	if mode == OutputReact {
		code.WriteString("\nexport function useSharedTasks(): SharedTaskState[] {\n")
		code.WriteString("    const client = useApiClient();\n")
		code.WriteString("    const [tasks, setTasks] = useState<SharedTaskState[]>([]);\n")
		code.WriteString("    useEffect(() => {\n")
		code.WriteString("        return client.onPush<TaskStateEvent>('TaskStateEvent', (event) => {\n")
		code.WriteString("            setTasks(event.tasks);\n")
		code.WriteString("        });\n")
		code.WriteString("    }, [client]);\n")
		code.WriteString("    return tasks;\n")
		code.WriteString("}\n\n")
		code.WriteString("export function useSharedTask(id: string): SharedTaskState | null {\n")
		code.WriteString("    const tasks = useSharedTasks();\n")
		code.WriteString("    return tasks.find(t => t.id === id) ?? null;\n")
		code.WriteString("}\n\n")
		code.WriteString("export function useMyTasks(): SharedTaskState[] {\n")
		code.WriteString("    const tasks = useSharedTasks();\n")
		code.WriteString("    return tasks.filter(t => t.isOwner && !t.parentId);\n")
		code.WriteString("}\n\n")
		code.WriteString("export function useTaskOutput(taskId: string): { lines: string[]; clear: () => void } {\n")
		code.WriteString("    const client = useApiClient();\n")
		code.WriteString("    const [lines, setLines] = useState<string[]>([]);\n")
		code.WriteString("    useEffect(() => {\n")
		code.WriteString("        return client.onPush<TaskUpdateEvent>('TaskUpdateEvent', (event) => {\n")
		code.WriteString("            if (event.taskId === taskId && event.output != null) {\n")
		code.WriteString("                setLines((prev) => [...prev, event.output!]);\n")
		code.WriteString("            }\n")
		code.WriteString("        });\n")
		code.WriteString("    }, [client, taskId]);\n")
		code.WriteString("    const clear = useCallback(() => setLines([]), []);\n")
		code.WriteString("    return { lines, clear };\n")
		code.WriteString("}\n")
	}

	if hasMeta {
		code.WriteString("\nexport interface TaskMeta {\n")
		code.WriteString("    userName?: string;\n")
		code.WriteString("    error?: string;\n")
		code.WriteString("}\n")
	}

	if hasHandlerFile {
		results["tasks.ts"] = code.String()
	} else {
		results["client.ts"] += code.String()
	}
}
...
func TestGenerateWithTasks(t *testing.T) {
	...
	// TaskNode always present (base protocol type)
	if !strings.Contains(output, "export interface TaskNode") {
		t.Error("Missing TaskNode interface")
	}

	// Shared task types from handler group reflection
	if !strings.Contains(output, "export interface SharedTaskState") {
		t.Error("Missing SharedTaskState interface")
	}

	// cancelSharedTask from convenience hook
	if !strings.Contains(output, "cancelSharedTask") {
		t.Error("Missing cancelSharedTask function")
	}

	// TaskNodeStatus should NOT appear as a duplicated enum
	if strings.Count(output, "export const TaskNodeStatus") > 1 {
		t.Error("TaskNodeStatus should only appear once, not duplicated")
	}
	...
	// onTaskStateEvent should not be duplicated
	...
	// No absent types
	if !strings.Contains(output, "export interface CancelTaskRequest") {
		t.Error("Missing CancelTaskRequest interface")
	}
}
...
func TestGenerateWithTasksMultiFile(t *testing.T) {
	...
	// TaskNode in base client
	if !strings.Contains(baseContent, "export interface TaskNode") {
		t.Error("Missing TaskNode interface in client.ts")
	}

	// task-cancel-handler.ts should exist (it's now a normal handler group)
	handlerContent, exists := files["task-cancel-handler.ts"]
	if !exists {
		t.Fatal("task-cancel-handler.ts should be generated as a normal handler file")
	}

	// SharedTaskState in handler file
	if !strings.Contains(handlerContent, "export interface SharedTaskState") {
		t.Error("Missing SharedTaskState in task-cancel-handler.ts")
	}

	// TaskStateEvent/TaskUpdateEvent interfaces in handler file
	if !strings.Contains(handlerContent, "export interface TaskStateEvent") {
		t.Error("Missing TaskStateEvent interface in task-cancel-handler.ts")
	}
	if !strings.Contains(handlerContent, "export interface TaskUpdateEvent") {
		t.Error("Missing TaskUpdateEvent interface in task-cancel-handler.ts")
	}

	// Push event handlers in handler file
	if !strings.Contains(handlerContent, "onTaskStateEvent") {
		t.Error("Missing onTaskStateEvent in task-cancel-handler.ts")
	}
	if !strings.Contains(handlerContent, "onTaskUpdateEvent") {
		t.Error("Missing onTaskUpdateEvent in task-cancel-handler.ts")
	}

	// tasks.ts should exist (from convenience hook)
	tasksContent, exists := files["tasks.ts"]
	if !exists {
		t.Fatal("tasks.ts should be generated by the OnGenerate hook")
	}
	if !strings.Contains(tasksContent, "cancelSharedTask") {
		t.Error("Missing cancelSharedTask in tasks.ts")
	}

	// cancelSharedTask should NOT be in client.ts
	if strings.Contains(baseContent, "cancelSharedTask") {
		t.Error("cancelSharedTask should be in tasks.ts, not client.ts")
	}
}
...
func TestGenerateWithTasksReact(t *testing.T) {
	...
	// Shared task hooks (from convenience hook)
	...
	// Shared task types from reflection
	if !strings.Contains(output, "export interface SharedTaskState") {
		t.Error("Missing SharedTaskState interface")
	}
...
func TestGenerateWithTasksReactMultiFile(t *testing.T) {
	...
	// task-cancel-handler.ts should exist
	handlerContent, exists := files["task-cancel-handler.ts"]
	if !exists {
		t.Fatal("task-cancel-handler.ts should be generated")
	}
	...
	// tasks.ts should exist with convenience code
	tasksContent, exists := files["tasks.ts"]
	if !exists {
		t.Fatal("tasks.ts should be generated by the OnGenerate hook")
	}
	...
	if !strings.Contains(tasksContent, "useSharedTasks") {
		t.Error("Missing useSharedTasks in tasks.ts")
	}
	if !strings.Contains(tasksContent, "useTaskOutput") {
		t.Error("Missing useTaskOutput in tasks.ts")
	}
}
...
func TestGenerateWithTaskMeta(t *testing.T) {
	...
	// Should have TaskMeta interface (from convenience hook)
	...
	// SharedTaskState has meta?: any from reflection (interface{} fields no longer skipped)
	if !strings.Contains(output, "meta?: any") {
		t.Error("Missing meta?: any field from reflection")
	}
...
func TestGenerateWithTaskMetaMultiFile(t *testing.T) {
	...
	// TaskMeta interface should be in tasks.ts (from convenience hook)
	tasksContent, ok := files["tasks.ts"]
	if !ok {
		t.Fatalf("Expected tasks.ts, got files: %v", mapKeys(files))
	}
	if !strings.Contains(tasksContent, "export interface TaskMeta") {
		t.Error("Missing TaskMeta interface in tasks.ts")
	}
}
...
func TestGenerateWithoutMetaHasAnyMetaField(t *testing.T) {
	...
	// interface{} fields now emit "any" instead of being skipped
	if !strings.Contains(output, "meta?: any") {
		t.Error("Expected meta?: any from reflection of interface{} fields")
	}
	if strings.Contains(output, "export interface TaskMeta") {

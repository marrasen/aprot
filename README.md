...
```go
registry.OverrideFieldType(api.AuditEvent{}, "Payload", api.OrderPayload{})
// generated: payload?: OrderPayload (interface declared like any other type)
```

The tasks subpackage uses this internally: `tasks.EnableWithMeta[MyMeta](registry)` types `TaskNode.meta` / `SharedTaskState.meta` as `MyMeta`'s generated interface, so client code reads `task.meta?.myField` without casts; plain `tasks.Enable` leaves them `unknown`.
...
A few type-mapping rules worth knowing: a bare pointer `*T` (no `json:,omitempty`) becomes `T | null` (it is always sent, as `null` when nil), while `*T` with `omitempty` stays optional `?: T`; a fixed-size array `[N]T` becomes a TypeScript tuple `[T, T, ...]` for `N ≤ 16` and plain `T[]` above that (`[4]float64` → `[number, number, number, number]`, with `z.tuple([...])` in the Zod schema and `minItems`/`maxItems` in OpenAPI), except `[N]byte`, which — like `[]byte` — is base64-encoded on the wire and becomes `string`; `map[bool]V` becomes `Partial<Record<"true" | "false", V>>`; `json.RawMessage` becomes `unknown`; and `time.Duration` becomes `number`, encoded as int64 nanoseconds.

A duration needs no struct tag. json/v2 has no default representation for one, so aprot sets nanoseconds — the encoding v1 `encoding/json` has always used — for every duration it marshals. An explicit `json:"d,format:sec"` (or `milli`, `micro`, `nano`) still wins and still generates `number`; `format:units` ("1m30s") and `format:iso8601` ("PT1M30S") generate `string`.

Do not add `format:nano` by hand. Before v0.61.0 aprot rejected a bare duration and asked for that tag, and the tag is not free: since Go 1.27, plain `encoding/json` rejects any struct holding a format-tagged field, because the v1 API cannot opt in to format tags. If you also persist the type with `encoding/json`, the tag breaks loading and saving it. Delete the tags you already have.
...
- **Mid-session refresh** — the same `auth` frame on a live connection updates the token/identity without reconnecting. A *failed* refresh keeps the existing session (a live connection is never downgraded) **and keeps the address and principal provider it already had**: if the hook called `conn.SetUserID` or `conn.SetPrincipalProvider` and then returned an error or panicked, aprot puts both back before sending `auth_error`, so a rejected refresh can't leave the connection resolving the principal the client was told it could not have. Those two fields are all it puts back — `conn.Set` values and your own stores stay as the hook left them, so a hook that does more should still clean up after itself.
...
Set `ServerOptions.Logger` (a `*slog.Logger`) to receive server-side error logs; nil uses `slog.Default()`. Currently logged: response-encode failures — a handler result that cannot be marshaled (e.g. a NaN float) is reported to the client as an internal error *and* logged with the method name, so operators see the failure even when no client surfaces it — and handler panics, logged with the method name, panic value, and stack trace. Clients only receive a generic message, so this log line is the only place the panic value and stack appear. `server.Logger()` returns the effective logger (the configured one, or `slog.Default()`), so adapters and middleware can write to the same sink.
...
The generated TypeScript client contains socket-reachable groups only: `RegisterREST`-only groups are skipped, since generated client functions for them would call over the socket where their methods deliberately do not exist. Consume REST-only handlers over HTTP (the OpenAPI spec covers them).
...
- **Registration modes**: every mode works with `EnableMCP`. For a group whose only surface should be MCP, use `RegisterMCP`: no WebSocket dispatch, no REST routes, no OpenAPI entry, and no generated TypeScript client functions — a curated model-facing tool group never becomes browser API surface. The adapter panics at construction if a `RegisterMCP` group has no tools enabled, since such a group would be reachable nowhere.
